...
const MORSE_HOLD_MS = 1200;
const MORSE_LONG_SUPPRESS_MS = 1300;
...
function handleD1(b){
  ...
  if(code===0x07){
    // A physical long-press produces a burst of repeated D1 07 packets.
    // The first two packets confirm the long press; remaining packets from
    // the same burst must not become extra dots.
    if(now < morse.playSuppressUntil){
      morse.dbg('D1 07 ignored (long-press repeat)');
      return true;
    }
    if(morse.pendingPlay && (now-morse.pendingPlay.at)<=MORSE_HOLD_MS){
      clearTimeout(morse.playTimer);
      morse.pendingPlay=null;
      morse.playSuppressUntil=now+MORSE_LONG_SUPPRESS_MS;
